...
@php
    ...
    /**
     * Overlay content for the server-rendered first slide. Every field is
     * optional: a slide with none of them renders exactly as an image-only
     * slide did, so slides saved before these fields existed are unaffected.
     */
    $firstSlide = $carouselImages[0] ?? [];

    $firstSlideHasContent = filled($firstSlide['heading'] ?? null)
        || filled($firstSlide['description'] ?? null)
        || filled($firstSlide['button_text'] ?? null);

    /**
     * Colours are stored hex-only, but they land in an inline `style`, so the
     * shape is re-checked here rather than trusted. Anything else falls back to
     * the theme's button colours.
     */
    $slideButtonStyle = function (array $slide): string {
        $styles = [];

        foreach (['button_color' => 'background-color', 'button_text_color' => 'color'] as $key => $property) {
            if (preg_match('/^#([0-9a-f]{3}|[0-9a-f]{6})$/i', (string) ($slide[$key] ?? ''))) {
                $styles[] = $property.':'.$slide[$key];
            }
        }
    ...
    <div class="overflow-hidden">
        @if ($firstImage)
            ...
            <div class="relative">
                ...
                @if ($firstSlideHasContent)
                    <div class="pointer-events-none absolute inset-0 flex items-center bg-gradient-to-r from-black/50 to-transparent rtl:bg-gradient-to-l">
                        <div class="mx-auto w-full max-w-[1440px] px-14 max-lg:px-8 max-sm:px-5">
                            <div class="max-w-[50%] max-lg:max-w-[65%] max-sm:max-w-full">
                                @if (filled($firstSlide['heading'] ?? null))
                                    <h2 class="text-[44px] font-bold leading-tight text-white max-lg:text-3xl max-sm:text-xl">
                                        {{ $firstSlide['heading'] }}
                                    </h2>
                                @endif

                                @if (filled($firstSlide['description'] ?? null))
                                    <p class="mt-4 max-w-[520px] text-lg text-white/90 max-lg:text-base max-sm:mt-2 max-sm:text-xs">
                                        {{ $firstSlide['description'] }}
                                    </p>
                                @endif

                                @if (filled($firstSlide['button_text'] ?? null))
                                    <a
                                        href="{{ $firstSlide['button_link'] ?? $firstSlide['link'] ?? '#' }}"
                                        class="pointer-events-auto mt-8 inline-block rounded bg-navyBlue px-8 py-3.5 text-base font-semibold text-white transition-all hover:opacity-90 max-sm:mt-4 max-sm:px-5 max-sm:py-2.5 max-sm:text-sm"
                                        style="{{ $slideButtonStyle($firstSlide) }}"
                                    >
                                        {{ $firstSlide['button_text'] }}
                                    </a>
                                @endif
                            </div>
                        </div>
        ...
        <div class="relative m-auto flex w-full overflow-hidden">
            ...
            >
                ...
                >
                    ...
                    {{--
                        Admin-managed overlay. `pointer-events-none` keeps the
                        slide's own drag/click handling intact; only the button
                        takes pointer events back.
                    --}}
                    <div
                        class="pointer-events-none absolute inset-0 flex items-center bg-gradient-to-r from-black/50 to-transparent rtl:bg-gradient-to-l"
                        v-if="hasContent(image)"
                    >
                        <div class="mx-auto w-full max-w-[1440px] px-14 max-lg:px-8 max-sm:px-5">
                            <div class="max-w-[50%] max-lg:max-w-[65%] max-sm:max-w-full">
                                <h2
                                    class="text-[44px] font-bold leading-tight text-white max-lg:text-3xl max-sm:text-xl"
                                    v-if="image.heading"
                                >
                                    @{{ image.heading }}
                                </h2>
                                ...
                                <p
                                    class="mt-4 max-w-[520px] text-lg text-white/90 max-lg:text-base max-sm:mt-2 max-sm:text-xs"
                                ...
                                <a
                                    ...
                                    :href="image.button_link || image.link || '#'"
    ...
    <script type="module">
        app.component("v-carousel", {
            ...
            methods: {
                ...
                hasContent(image) {
                    return !! (image.heading || image.description || image.button_text);
                },

                /**
                 * Admin-picked button colours, falling back to the theme's own
                 * button styling when a slide leaves them unset.
                 */
                buttonStyle(image) {
                    const style = {};

                    if (image.button_color) {
                        style.backgroundColor = image.button_color;
                    }

                    if (image.button_text_color) {
